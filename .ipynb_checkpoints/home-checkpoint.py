...
def app():
    ...
    value1 = st.slider(label = 'Select Age:' , min_value=18 , max_value=65 , value=25 , step = 1)
    ...
    if st.button('Predict'):
        sex_encoder.transform([value2])
        smoker_encoder.transform([value5])
        region_encoder.transform([value6])
        input = np.array([value1 , value2 , value3 , value4 , value5 , value6])
        scaler.transform(input)
        cost = model.predict(input)
        st.success(f'The Predict Medical Cost is {cost}')